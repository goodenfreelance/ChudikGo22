...
type Creature struct {
	...
	InvulnerableSec      float64           `json:"invulnerableSec"`
	InvulnerableUntil    time.Time         `json:"-"`
...
type WSInputMessage struct {
	...
	ActivateInvulnerability bool              `json:"activateInvulnerability,omitempty"`
...
func ToCreatureNet(c Creature) CreatureNet {
	...
	isInv := c.IsInvulnerable || (!c.InvulnerableUntil.IsZero() && time.Now().Before(c.InvulnerableUntil))
	invSec := 0.0
	if isInv && !c.InvulnerableUntil.IsZero() {
		invSec = math.Max(0, c.InvulnerableUntil.Sub(time.Now()).Seconds())
	}
	return CreatureNet{
		ID:              c.ID,
		Name:            c.Name,
